...
// Predefined struct for user
type VerifyPurchaseRequestParams struct {
...
func (r *VerifyPurchaseRequest) ToJsonString() string {
    b, _ := json.Marshal(r)
    return string(b)
}
...
// FromJsonString It is highly **NOT** recommended to use this function
// because it has no param check, nor strict type check
func (r *VerifyPurchaseRequest) FromJsonString(s string) error {
	f := make(map[string]interface{})
	if err := json.Unmarshal([]byte(s), &f); err != nil {
		return err
	}
	...
	if len(f) > 0 {
		return tcerr.NewTencentCloudSDKError("ClientError.BuildRequestError", "VerifyPurchaseRequest has unknown keys!", "")
	}
	return json.Unmarshal([]byte(s), &r)
}

// Predefined struct for user
type VerifyPurchaseResponseParams struct {
	// 唯一请求 ID，每次请求都会返回。定位问题时需要提供该次请求的 RequestId。
	RequestId *string `json:"RequestId,omitempty" name:"RequestId"`
}
...
type VerifyPurchaseResponse struct {
	*tchttp.BaseResponse
	Response *VerifyPurchaseResponseParams `json:"Response"`
}

func (r *VerifyPurchaseResponse) ToJsonString() string {
    b, _ := json.Marshal(r)
    return string(b)
}
...
// FromJsonString It is highly **NOT** recommended to use this function
// because it has no param check, nor strict type check
func (r *VerifyPurchaseResponse) FromJsonString(s string) error {
	return json.Unmarshal([]byte(s), &r)
}