...
type AutoSnapshotPolicyInfo struct {
	...
	// 跨地域复制的快照保留时间，单位天
	CrossRegionsAliveDays *uint64 `json:"CrossRegionsAliveDays,omitnil" name:"CrossRegionsAliveDays"`
}
...
type CreateCfsFileSystemRequestParams struct {
	...
	EnableAutoScaleUp *bool `json:"EnableAutoScaleUp,omitnil" name:"EnableAutoScaleUp"`
}
...
type DescribeUserQuotaResponseParams struct {
	...
	// UserQuota条目
	UserQuotaInfo []*UserQuota `json:"UserQuotaInfo,omitnil" name:"UserQuotaInfo"`
...
type SetUserQuotaRequestParams struct {
	...
	// 容量硬限制，单位GiB
	CapacityHardLimit *uint64 `json:"CapacityHardLimit,omitnil" name:"CapacityHardLimit"`

	// 文件硬限制，单位个
	FileHardLimit *uint64 `json:"FileHardLimit,omitnil" name:"FileHardLimit"`
}
...
type UserQuota struct {
	...
	// 文件使用个数，单位个
	// 注意：此字段可能返回 null，表示取不到有效值。
	FileUsed *uint64 `json:"FileUsed,omitnil" name:"FileUsed"`
}