...
type CreateAudioModerationSyncTaskResponseParams struct {
	...
	// 音频中对话内容审核结果；
	// 注意：此字段可能返回 null，表示取不到有效值。
	TextResults []*TextResult `json:"TextResults,omitnil" name:"TextResults"`

	// 音频中低俗内容审核结果；
	// 注意：此字段可能返回 null，表示取不到有效值。
	MoanResults []*MoanResult `json:"MoanResults,omitnil" name:"MoanResults"`
	...
	// 音频中说话人识别返回结果；
	// 注意：此字段可能返回 null，表示取不到有效值。
	SpeakerResults []*AudioResultDetailSpeakerResult `json:"SpeakerResults,omitnil" name:"SpeakerResults"`
