...
// Predefined struct for user
type GetRunJobLogRequestParams struct {
	// <p>任务Uuid。</p>
	RunUuid *string `json:"RunUuid,omitnil,omitempty" name:"RunUuid"`
...
type GetRunJobLogRequest struct {
	*tchttp.BaseRequest
...
func (r *GetRunJobLogRequest) ToJsonString() string {
    b, _ := json.Marshal(r)
    return string(b)
}
...
// FromJsonString It is highly **NOT** recommended to use this function
// because it has no param check, nor strict type check
func (r *GetRunJobLogRequest) FromJsonString(s string) error {
	f := make(map[string]interface{})
	if err := json.Unmarshal([]byte(s), &f); err != nil {
		return err
	}
	...
	delete(f, "Path")
	if len(f) > 0 {
		return tcerr.NewTencentCloudSDKError("ClientError.BuildRequestError", "GetRunJobLogRequest has unknown keys!", "")
	}
	return json.Unmarshal([]byte(s), &r)
}

// Predefined struct for user
type GetRunJobLogResponseParams struct {
	// <p>日志内容</p>
	Content *string `json:"Content,omitnil,omitempty" name:"Content"`
...
type GetRunJobLogResponse struct {
	*tchttp.BaseResponse
	Response *GetRunJobLogResponseParams `json:"Response"`
}

func (r *GetRunJobLogResponse) ToJsonString() string {
    b, _ := json.Marshal(r)
    return string(b)
}
...
// FromJsonString It is highly **NOT** recommended to use this function
// because it has no param check, nor strict type check
func (r *GetRunJobLogResponse) FromJsonString(s string) error {
	return json.Unmarshal([]byte(s), &r)
}
