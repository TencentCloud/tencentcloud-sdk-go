...
func main() {
	secretID := "AKIDA7nBITs4D7l9G7BAn9IBnAIdjZmzBe6H"
	secretKey := "WmmVs0sToIXVDhwvf8rMbZDcldE0Wmm3"

	credential := common.NewCredential(secretID, secretKey)
	// 实例化一个客户端配置对象，可以指定超时时间等配置
	cpf := profile.NewClientProfile()
	// 实例化要请求产品(以cvm为例)的client对象
	ticClient, err := tic.NewClient(credential, "" /* 不需要考虑地域 */, cpf)
	if err != nil {
	...
	createStackReq.StackRegion = common.StringPtr("ap-guangzhou")
	createStackReq.TemplateUrl = common.StringPtr("https://hello-1302774858.cos.ap-guangzhou.myqcloud.com/main.tf.zip")
